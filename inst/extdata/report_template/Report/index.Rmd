...
lapply(
  c('antgreens', 'knitr', 'tidyverse', 'RColorBrewer', 'data.table', 'readxl', 'Rcpp', 'rlang', 'webshot', 'DT', 'echarts4r'), 
  function(pkg) {
...
dims <- c(names(params$ana_dims_part), "product", "drug")
dims_comb_data <- gen_dims_comb_stats(data, dims, fast = TRUE)
```
...
&emsp;&emsp;确定分析的所有维度，并为其中文名称指定英文变量名，放入列表中，例如：

```markdown
dims_en2cn <- list(year = "年份", ...)
```

**antgreens** 默认使用 `r paste0(sapply(names(RawColnameTrans), \(x) {paste0(x, " = ", RawColnameTrans[[x]])}), collapse = '、')` 作为分析的维度以及其名称，其中中文名为原始数据中的列名，英文名为运行时使用的变量名。
...
&emsp;&emsp;载入原始数据 `data`，确定需要分析的所有维度名（即列名），使用 `check_sample_size(data, dimensions, path)` 获取各维度组合下的样本量（最大、最小、中位数），并保存在文件中；然后手动根据实际情况确定各维度组合的样本量阈值，确定阈值时可以使用
...
&emsp;&emsp;将确定好的阈值写入之前输出的文件，列名建议包含“阈值”一词（例如“样本量阈值”），也可以取其他名称，但是需要记录阈值所在列的列名关键词：
